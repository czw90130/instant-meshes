...
int main(int argc, char **argv)
{
    ...
    if (args.size() > 1 || help || (!batchOutput.empty() && args.size() == 0))
    {
        ...
        cout << "   -d, --deterministic       Prefer (slower) deterministic algorithms 优先使用（较慢的）确定性算法" << endl;
        cout << "   -c, --crease <degrees>    Dihedral angle threshold for creases 褶皱边缘二面角的阈值" << endl;
        cout << "   -S, --smooth <iter>       Number of smoothing & ray tracing reprojection steps (default: 2) 平滑和光线跟踪的重新投影步骤数（默认值：2）" << endl;
        cout << "   -D, --dominant            Generate a tri/quad dominant mesh instead of a pure tri/quad mesh 生成三/四主导网格而不是纯三/四网格" << endl;
        cout << "   -i, --intrinsic           Intrinsic mode (extrinsic is the default) 内部模式（默认为外部）" << endl;
        cout << "   -b, --boundaries          Align to boundaries (only applies when the mesh is not closed) 对齐边界（仅在未关闭网格时适用）" << endl;
        cout << "   -r, --rosy <number>       Specifies the orientation symmetry type (2, 4, or 6) 指定方向对称类型（2、4或6）" << endl;
        cout << "   -p, --posy <number>       Specifies the position symmetry type (4 or 6) 指定位置对称类型（4或6）" << endl;
        cout << "   -s, --scale <scale>       Desired world space length of edges in the output 输出中边缘的所需世界空间长度" << endl;
        cout << "   -f, --faces <count>       Desired face count of the output mesh 输出网格的所需面数" << endl;
        cout << "   -v, --vertices <count>    Desired vertex count of the output mesh 输出网格所需的顶点数" << endl;
        cout << "   -C, --compat              Compatibility mode to load snapshots from old software versions 兼容模式可从旧软件版本加载快照" << endl;
        cout << "   -k, --knn <count>         Point cloud mode: number of adjacent points to consider 点云模式：要考虑的相邻点数" << endl;
